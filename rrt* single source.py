...
class RRTmap:
    ...
    def findstart(self):
        ...
        for i in range(r):
            for j in range(c):
                if (self.img[i][j][1] > 200 and self.img[i][j][0] < 100 and self.img[i][j][0] < 100):
    ...
    def findend(self):
        ...
        for i in range(r):
            for j in range(c):
                if (self.img[i][j][2] > 200 and self.img[i][j][1] < 100 and self.img[i][j][0] < 100):
...
img = cv2.imread("singlesourcemap.png", 1)
map = RRTmap(img)
map.findstart()
map.findend()
